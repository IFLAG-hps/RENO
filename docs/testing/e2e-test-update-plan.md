...
- LocalStack上での主要API動作確認
- 利用状況取得（利用回数の運用調整は後続対応）
...
6. 機能面の更新が入るたびに、影響するE2Eシナリオを見直し、必要なテストを追加・修正する。
...
1. 認証なしでのチャット相談 → 写真選択（サンプル画像）→ 診断結果表示
2. 写真選択 → 施工後イメージ表示 → 昼夜切り替え
3. 費用シミュレーター → 条件変更 → 最終確認
4. 最終確認 → PDF保存 → 担当者相談フォーム表示
5. 担当者相談フォームの未入力エラーと受付完了
...
4. チャット、認可、主要APIのバックエンドテストを拡張する。利用回数の調整は後続対応とする。
5. CIでP0を必須化し、機能面の更新があるたびに関連E2Eを見直す。
...
- 認証なしでのチャット利用開始から主要導線まで
- 写真選択、診断、施工後イメージ表示
- 費用シミュレーターから最終確認まで
- PDF保存と担当者相談フォーム
- 事例・履歴・管理者機能の代表的な画面操作
- 主要な入力エラーとAPIエラー時の復帰

DOM完全一致テストはReact移行期間の回帰確認として残すが、最終的な品質判定は、表示結果・操作結果・API呼び出し結果を検証するE2Eを中心にする。機能面の更新があるたびに、影響範囲を確認してE2Eを更新する。

## 10. テスト実装の優先順位

| 順序 | テスト | 実施内容 | リリース上の扱い |
| --- | --- | --- | --- |
| 1 | バックエンド単体 | トークン、入力検証、費用計算 | 直ちに必須化 |
| 2 | API・統合 | 認証、認可、保存・削除 | 対象APIの変更時に必須化 |
| 3 | 主要E2E | チャット、画像、PDF、相談 | P0として必須化 |
...
- `backend/tests`にトークン、入力検証の単体テストを追加する。
- `test_localstack.py`に認証なしの主要導線、認可、事例CRUD、URL発行のテストを追加する。
